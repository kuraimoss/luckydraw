...
class LuckyDraw {
    ...
    async loadParticipants() {
        try {
            ...
            this.participants = data.map((value, index) => ({
                id: index,
                name: value.toString(),
                value: value,
            ...
            // Use default participants if API fails
            const defaultValues = [1, 3, 7, 20, 50, 75, 100, 500];
            this.participants = defaultValues.map((value, index) => ({
                id: index,
                name: value.toString(),
                value: value,
                color: this.colors[index % this.colors.length]
    ...
    async spin() {
        ...
        // Get winner from backend - SINGLE SOURCE OF TRUTH
        try {
            ...
            const response = await fetch('/api/spin', {
                ...
                body: JSON.stringify({ participants: participantValues })
            });

            if (!response.ok) {
                throw new Error('Failed to get spin result');
            }

            const result = await response.json();
            const winnerIndex = result.winnerIndex;
            const winner = this.participants[winnerIndex];
            const finalAngle = result.finalAngle;

            console.log(`🎯 Winner: ${winner.value} (index ${winnerIndex})`);

            // Animate spin to the final angle determined by backend
